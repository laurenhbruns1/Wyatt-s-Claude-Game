export const BADGE_DEFS = {
  ...
  region_sweep: { label: 'Region Sweep', blurb: 'All five picks came from the same region.' },
  chapter_purist: { label: 'Chapter Purist', blurb: 'All five picks debuted the same chapter.' },
  rotation_survivor: {
...
export function computeBadges({ squad, result, mode }) {
  ...
  if (players.length === 5 && players.every((p) => p.region === players[0].region)) {
    badges.push('region_sweep')
  }
  if (players.length === 5 && players.every((p) => p.chapter === players[0].chapter)) {
